...
export class ProfilePage {
    ...
    constructor(
        ...
        if(!this.navParams.get('user') && this.navParams.get('id')){
            this.user = {id: this.navParams.get('id')};
        }
        console.log(this.user);
        this.api.storage.get('user_id').then((val) => {
            if(!this.user.id || this.user.id && val == this.user.id || this.user.id == 0){
                this.pageData.title = 'הפרופיל שלי';
                this.user.id = val;
                this.api.showLoad();
            }else{
                this.pageData.title = 'הפרופיל של ' + (this.user.userfName ? this.user.userfName : '');
            }
            this.getProfileData();
        });
